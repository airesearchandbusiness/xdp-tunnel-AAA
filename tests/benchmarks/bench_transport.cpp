...
static void BM_Wrap(benchmark::State &state, int tid) {
    ...
    if (!transport_get(id)) { state.SkipWithMessage("transport unavailable"); return; }
    size_t sz = static_cast<size_t>(state.range(0));
    std::vector<uint8_t> in(sz, 0xAB), out(4096);
    FrameContext ctx{}; ctx.seq = 0; ctx.sni = "example.com"; ctx.conn_id_len = 8;
...
static void BM_Unwrap(benchmark::State &state, int tid) {
    ...
    if (!wr.ok) { state.SkipWithMessage("wrap failed"); return; }
    for (auto _ : state) {
