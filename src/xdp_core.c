...
int xdp_tx_path(struct xdp_md *ctx) {
    ...
    *tx_seq_ptr = local_seq + 1;
    ...
    /* obfs flags must be available before the QUIC mimicry padding block
     * below (which reads obfs at the TACHYON_OBFS_CONST_PAD check). CWE-457. */
    ...
    if (mimicry == TACHYON_MIMICRY_QUIC) {
        ...
        if (max_pad < TACHYON_MAX_FRAME_LEN) {
...
#pragma unroll
     ...
     * Hold replay_lock during the multi-field update so concurrent CPUs
     * processing different roaming events cannot produce a torn write
     * (e.g., new IP with old MAC). CWE-362. */
    bpf_spin_lock(&sess->replay_lock);
...
/* ======================================================================
 * Key Injection Syscall Programs
 * ====================================================================== */
