...
size_t build_stun_message(uint8_t *out, size_t cap, uint16_t msg_type, const uint8_t txn_id[12],
    ...
    /* STUN header */
    put_u16(out + off, msg_type);
    off += 2;
    put_u16(out + off, static_cast<uint16_t>(body_len));
    off += 2;
    put_u32(out + off, STUN_MAGIC_COOKIE);
    off += 4;
    std::memcpy(out + off, txn_id, 12);
    off += 12;

    /* DATA attribute */
    put_u16(out + off, ATTR_DATA);
    off += 2;
    put_u16(out + off, static_cast<uint16_t>(payload_len));
    off += 2;
...
static tachyon::transport::FrameResult stun_wrap(const uint8_t *payload, size_t payload_len,
...
static tachyon::transport::FrameResult stun_unwrap(const uint8_t *frame, size_t frame_len,
                                                   uint8_t *out, size_t out_cap) {
    using tachyon::transport::FrameResult;
...
static int stun_score(const tachyon::transport::EnvProfile &env) {
    ...
    if (env.udp)
        s += 20;
    if (env.port == 3478 || env.port == 19302)
        s += 30;
    if (env.port == 443)
        s += 5;
...
void register_transport() {
    tachyon::transport::transport_register(&stun_ops);
}
