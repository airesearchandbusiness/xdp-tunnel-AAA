...
FrameHeader parse_frame_header(const uint8_t *buf, size_t len) {
    ...
    if (len < H2_FRAME_HEADER)
        return h;
    h.length =
        (static_cast<uint32_t>(buf[0]) << 16) | (static_cast<uint32_t>(buf[1]) << 8) | buf[2];
    h.type = buf[3];
    h.flags = buf[4];
    h.stream_id = (static_cast<uint32_t>(buf[5] & 0x7F) << 24) |
                  (static_cast<uint32_t>(buf[6]) << 16) | (static_cast<uint32_t>(buf[7]) << 8) |
                  buf[8];
    h.ok = true;
    return h;
}
...
size_t build_connection_preface(uint8_t *out, size_t cap) {
    ...
    std::memcpy(out + H2_PREFACE_LEN + H2_FRAME_HEADER, SETTINGS_PAYLOAD, sizeof(SETTINGS_PAYLOAD));
    return need;
}
...
static tachyon::transport::FrameResult http2_wrap(const uint8_t *payload, size_t payload_len,
    ...
    if (ctx->seq == 0) {
        ...
        const size_t hdr_len =
            build_headers_frame(out + preface_len, out_cap - preface_len, ctx->sni, sid);
        if (hdr_len == 0)
            return {0, false};
        const size_t data_len =
            build_data_frame(out + preface_len + hdr_len, out_cap - preface_len - hdr_len, payload,
                             payload_len, sid);
        if (data_len == 0)
            return {0, false};
...
static tachyon::transport::FrameResult http2_unwrap(const uint8_t *frame, size_t frame_len,
                                                    uint8_t *out, size_t out_cap) {
    using tachyon::transport::FrameResult;
    ...
    if (frame_len >= H2_PREFACE_LEN && std::memcmp(frame, CLIENT_PREFACE, H2_PREFACE_LEN) == 0)
        off += H2_PREFACE_LEN;
...
void register_transport() {
    tachyon::transport::transport_register(&http2_ops);
}
